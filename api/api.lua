...
function zUI.api.zSkin(f, offset, square, _type)
	...
	for i = 1, 4 do
		...
		x:SetTexture('Interface\\AddOns\\rexUI\\img\\borders\\'.._type..'border-'..section..square..'.tga')
        t[sections[i]] = x
    end
	...
	for i = 5, 8 do
		local section = sections[i]
        local x = f:CreateTexture(nil, 'OVERLAY', nil, 1)
		x:SetTexture('Interface\\AddOns\\rexUI\\img\\borders\\'.._type..'border-'..section..'.tga');
		t[sections[i]] = x
	end
...
function zUI.api.CreateScrollFrame(name, parent)
	...
	--f.slider:SetThumbTexture("Interface\\AddOns\\rexUI\\img\\col")
...
function zUI.api.CreateBackdrop(f, inset, legacy, transp, backdropSetting)
  ...
  if not f.backdrop_shadow and zUI_config.appearance.border.shadow == "1" then
    ...
    f.backdrop_shadow:SetBackdrop({
      edgeFile = "Interface\\AddOns\\rexUI\\img\\glow2", edgeSize = size,
...
function zUI.api.CreateInfoBox(text, time, parent, height)
  ...
  if not infobox then
    ...
    --infobox.timeout:SetStatusBarTexture("Interface\\AddOns\\rexUI\\img\\bar")
