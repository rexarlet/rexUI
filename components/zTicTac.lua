...
zUI:RegisterComponent("zTicTac", function ()
	...
	zUI.zTicTac.bgTex:SetTexture("Interface\\AddOns\\rexUI\\img\\zTicTacBG");
	...
	function zUI.SetupGameButton(b, pointX, pointY, msgToSend, reference)
		...
		b:SetNormalTexture("");
		b:SetHighlightTexture("Interface\\AddOns\\rexUI\\img\\zTicTacHL");
		b:SetScript('OnClick', function() 
		if (myTurn == false) then zPrint("Not your turn!") return end
		if (gameMaster == true) then 
			b:SetDisabledTexture("Interface\\AddOns\\rexUI\\img\\zTicTacX");
			b.state = "x"
		else
			b:SetDisabledTexture("Interface\\AddOns\\rexUI\\img\\zTicTacO");
			b.state = "o"
		end
		...
		for index, v in ipairs(gameTable) do
			if (arg1 == tostring(index)) then 
				...
				if (gameMaster == false) then 
					v:SetDisabledTexture("Interface\\AddOns\\rexUI\\img\\zTicTacX");
					v.state = "x"
					v:SetAlpha(1); 
					v:Disable(); 
					break
				else
					v:SetDisabledTexture("Interface\\AddOns\\rexUI\\img\\zTicTacO");
					v.state = "o"
					v:SetAlpha(1); 
					v:Disable();
					break
